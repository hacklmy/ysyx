...
class IDU extends Module{
    ...
    when(br_taken_cancel && ds_allowin){
        ds_valid := false.B
    }.elsewhen(ds_allowin){
        ds_valid := io.fs_to_ds_valid
    }
    when(io.fs_to_ds_valid&&ds_allowin){
        ds_pc := io.pc
    ...
    val csr_wdata = Lookup(inst, 0.U, Array(
        csrrw -> io.rdata1,
        csrrs -> (io.rdata1 | csr.rdata),
        csrrc -> (io.rdata1 & (~csr.rdata)),
    ))
    ...
    csr.waddr2 := 3.U
    csr.wdata2 := io.rdata2
    

    src1 := Mux(src1_is_pc, ds_pc, io.rdata1)
    ...
    br_taken := Lookup(inst, false.B, Array(
        jal -> true.B ,
        jalr -> true.B ,
        bne -> Mux(io.rdata1.asSInt =/= io.rdata2.asSInt, true.B , false.B),
        beq -> Mux(io.rdata1.asSInt === io.rdata2.asSInt, true.B , false.B),
        bge -> Mux(io.rdata1.asSInt >= io.rdata2.asSInt, true.B , false.B),
        blt -> Mux(io.rdata1.asSInt < io.rdata2.asSInt, true.B , false.B),
        bltu -> Mux(io.rdata1 < io.rdata2, true.B , false.B),
        bgeu -> Mux(io.rdata1 >= io.rdata2, true.B , false.B),
        ecall -> true.B,
        mret -> true.B
    ))
    ...
    //printf("ds_pc:%x ds_valid:%d br_taken:%d src1:%x src2:%x  wen:%d wmask:%x\n",ds_pc,ds_valid,br_taken,io.src1,io.src2,Writemem_en,Wmask)
