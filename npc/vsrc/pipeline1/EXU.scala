...
class EXU extends Module{
    ...
    alu_res := MuxLookup(inst_now, 0.U, Array(
        ADDI.U -> add_res,
        AUIPC.U -> add_res,
        LUI.U -> src2_value,
        JAL.U -> (es_pc+4.U),
    ...
    printf("es_pc:%x es_valid:%d alu_res:%x src1_value:%x  src2_value:%x\n",es_pc,es_valid,alu_res,src1_value,src2_value)
    // io.Waddr := add_res
    // io.to_ms_store_data := store_data
    // io.Wmask := st_wstrb
    // io.Write_en := st_we
