...
class D_CACHE extends Module{
    ...
    val ram_0 = RegInit(VecInit(Seq.fill(16)(0.U(512.W))))
    val ram_1 = RegInit(VecInit(Seq.fill(16)(0.U(512.W))))
    ...
    switch(state){
        is(idle){
            when(io.from_lsu.arvalid){
                when(io.from_lsu.araddr >= "ha0000000".U){
                    state := r_mem
                }.otherwise{
                    state := r_lookup
                }
        ...
        is(reload){
            when(unuse_way===1.U){
                state := lookup_wait
                ram_0(index) := Cat(receive_data(7),receive_data(6),receive_data(5),receive_data(4),receive_data(3),receive_data(2),receive_data(1),receive_data(0))
                tag_0(index) := tag
                valid_0(index) := 1.U
                quene := 1.U
            }.elsewhen(unuse_way===2.U){
                state := lookup_wait
                ram_1(index) := Cat(receive_data(7),receive_data(6),receive_data(5),receive_data(4),receive_data(3),receive_data(2),receive_data(1),receive_data(0))
                tag_1(index) := tag
                valid_1(index) := 1.U
                quene := 0.U
            }.otherwise{
                when(quene===0.U){
                    ...
                    when(dirty_0(index)===1.U){
                        ...
                        write_back_addr := Cat(tag_0(index) , index, Fill(6,0.U))
                        ram_0(index) := Cat(receive_data(7),receive_data(6),receive_data(5),receive_data(4),receive_data(3),receive_data(2),receive_data(1),receive_data(0))
                        tag_0(index) := tag
                        dirty_0(index) := 0.U
                        valid_0(index) := 1.U
                    ...
                    when(dirty_1(index)===1.U){
                        ...
                        write_back_addr := Cat(tag_1(index) , index, Fill(6,0.U))
                        ram_1(index) := Cat(receive_data(7),receive_data(6),receive_data(5),receive_data(4),receive_data(3),receive_data(2),receive_data(1),receive_data(0))
                        tag_1(index) := tag
                        dirty_1(index) := 0.U
                        valid_1(index) := 1.U
    ...
    when(state===r_lookup){
        ...
        io.to_axi.araddr := io.from_lsu.araddr  & "hffffffffffffffc0".U
        io.to_axi.arlen := 7.U
        io.to_axi.arsize := 6.U
