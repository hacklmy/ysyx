...
class I_CACHE extends Module{
    ...
    val index = io.from_ifu.araddr(9,6)
    val tag = io.from_ifu.araddr(31,10)
    ...
    when(state===lookup){
        ...
        io.to_axi.bready := false.B
        when(way0_hit){
                io.to_ifu.rdata := ram_0(index) >> shift_bit
                io.to_ifu.arready := true.B
                io.to_ifu.rvalid := true.B
                io.to_ifu.rlast := true.B
                io.to_ifu.wready := false.B
                io.to_ifu.awready := false.B
                io.to_ifu.bvalid := false.B
                
            }.elsewhen(way1_hit){
                io.to_ifu.rdata := ram_1(index) >> shift_bit
                io.to_ifu.arready := true.B
                io.to_ifu.rvalid := true.B
                io.to_ifu.rlast := true.B
                io.to_ifu.wready := false.B
        ...
        io.to_axi.araddr := io.from_ifu.araddr  & "hffffffffffffffc0".U
        io.to_axi.arlen := 7.U
        io.to_axi.arsize := 6.U
