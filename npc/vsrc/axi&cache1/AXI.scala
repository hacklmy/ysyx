...
class AXI extends Module{
    ...
    val axi_wready = RegInit(0.U(1.W))
    ...
    Mem_modle.io.Raddr := araddr
    Mem_modle.io.Waddr := awaddr
    Mem_modle.io.Wdata := io.axi_in.wdata
    Mem_modle.io.Wmask := io.axi_in.wstrb
    Mem_modle.io.Write_en := axi_wready
    Mem_modle.io.Read_en := axi_rvalid
    ...
    switch(state){
        is(idle){
            when(io.axi_in.awvalid && io.axi_in.wvalid){
                ...
                axi_wready := 1.U
                //axi_bvalid := 1.U
                awlen := io.axi_in.awlen
                awaddr := io.axi_in.awaddr
                ...
                state := r_state
                arlen := io.axi_in.arlen
                araddr := io.axi_in.araddr
                axi_arready := 0.U
                axi_rvalid := 1.U
            }
        }
    ...
    io.axi_out.rlast := state===r_state && arlen===0.U
