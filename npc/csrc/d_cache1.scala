...
class D_CACHE extends Module{
    ...
    val idle :: r_lookup :: w_lookup ::  r_miss :: w_miss :: reload :: write_back :: r_mem :: w_mem :: Nil = Enum(9)
    //val w_idle :: write :: Nil = Enum(2)

    val offset = io.from_lsu.araddr(3,0)
    val index = io.from_lsu.araddr(7,4)
    val tag = io.from_lsu.araddr(31,8)

    val cacheLine = Mem(64, UInt(128.W)) // 64行，每行16字节
    ...
    val write_back_data = RegInit(0.U(128.W))
    ...
    val quene = RegInit(VecInit(Seq.fill(16)(0.U(8.W))))
    ...
    switch(state){
        ...
        is(w_lookup){
            when(anyMatch){
                ...
                cacheLine(tagIndex) := ((io.from_lsu.wdata & wmask) << shift_bit) | (cacheLine(tagIndex) & ~(wmask << shift_bit))
        ...
        is(reload){
            when(!allvalid){
                ...
                when(dirtyMem(replaceIndex)===1.U){
                    ...
                    write_back_addr := Cat(tagMem(replaceIndex) , index, Fill(4,0.U))
    ...
    when(state===r_lookup){
        ...
        io.to_lsu.rdata := cacheLine(tagIndex) >> shift_bit
        ...
        io.to_axi.araddr := io.from_lsu.araddr  & "hfffffffffffffff0".U
        io.to_axi.arlen := 1.U
        io.to_axi.arsize := 6.U
        ...
        io.to_axi.wvalid := false.B
        io.to_axi.bready := false.B
    }.elsewhen(state===w_miss || state === w_mem){
        io.to_lsu.rdata := 0.U
        io.to_lsu.arready := false.B
        io.to_lsu.rvalid := false.B
        io.to_lsu.rlast := false.B
    ...
    }.elsewhen(state===r_mem){
        io.to_lsu := io.from_axi
        io.to_axi := io.from_lsu
    }.otherwise{
        when(state===idle && (((io.from_lsu.arvalid||io.from_lsu.awvalid)&& (io.from_lsu.araddr >= "ha0000000".U)))){
            io.to_lsu := io.from_axi
            io.to_axi := io.from_lsu
        }.otherwise{
            io.to_lsu.rdata := 0.U
