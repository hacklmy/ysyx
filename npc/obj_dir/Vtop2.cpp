// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Model implementation (design independent parts)

#include "Vtop2.h"
#include "Vtop2__Syms.h"
#include "verilated_vcd_c.h"

//============================================================
// Constructors

Vtop2::Vtop2(VerilatedContext* _vcontextp__, const char* _vcname__)
    : vlSymsp{new Vtop2__Syms(_vcontextp__, _vcname__, this)}
    , clock{vlSymsp->TOP.clock}
    , reset{vlSymsp->TOP.reset}
    , io_inst{vlSymsp->TOP.io_inst}
    , io_pc{vlSymsp->TOP.io_pc}
    , io_outval{vlSymsp->TOP.io_outval}
    , rootp{&(vlSymsp->TOP)}
{
}

Vtop2::Vtop2(const char* _vcname__)
    : Vtop2(nullptr, _vcname__)
{
}

//============================================================
// Destructor

Vtop2::~Vtop2() {
    delete vlSymsp;
}

//============================================================
// Evaluation loop

void Vtop2___024root___eval_initial(Vtop2___024root* vlSelf);
void Vtop2___024root___eval_settle(Vtop2___024root* vlSelf);
void Vtop2___024root___eval(Vtop2___024root* vlSelf);
QData Vtop2___024root___change_request(Vtop2___024root* vlSelf);
#ifdef VL_DEBUG
void Vtop2___024root___eval_debug_assertions(Vtop2___024root* vlSelf);
#endif  // VL_DEBUG
void Vtop2___024root___final(Vtop2___024root* vlSelf);

static void _eval_initial_loop(Vtop2__Syms* __restrict vlSymsp) {
    vlSymsp->__Vm_didInit = true;
    Vtop2___024root___eval_initial(&(vlSymsp->TOP));
    // Evaluate till stable
    int __VclockLoop = 0;
    QData __Vchange = 1;
    vlSymsp->__Vm_activity = true;
    do {
        VL_DEBUG_IF(VL_DBG_MSGF("+ Initial loop\n"););
        Vtop2___024root___eval_settle(&(vlSymsp->TOP));
        Vtop2___024root___eval(&(vlSymsp->TOP));
        if (VL_UNLIKELY(++__VclockLoop > 100)) {
            // About to fail, so enable debug to see what's not settling.
            // Note you must run make with OPT=-DVL_DEBUG for debug prints.
            int __Vsaved_debug = Verilated::debug();
            Verilated::debug(1);
            __Vchange = Vtop2___024root___change_request(&(vlSymsp->TOP));
            Verilated::debug(__Vsaved_debug);
            VL_FATAL_MT("vsrc/top2.v", 1, "",
                "Verilated model didn't DC converge\n"
                "- See https://verilator.org/warn/DIDNOTCONVERGE");
        } else {
            __Vchange = Vtop2___024root___change_request(&(vlSymsp->TOP));
        }
    } while (VL_UNLIKELY(__Vchange));
}

void Vtop2::eval_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+++++TOP Evaluate Vtop2::eval_step\n"); );
#ifdef VL_DEBUG
    // Debug assertions
    Vtop2___024root___eval_debug_assertions(&(vlSymsp->TOP));
#endif  // VL_DEBUG
    // Initialize
    if (VL_UNLIKELY(!vlSymsp->__Vm_didInit)) _eval_initial_loop(vlSymsp);
    // Evaluate till stable
    int __VclockLoop = 0;
    QData __Vchange = 1;
    vlSymsp->__Vm_activity = true;
    do {
        VL_DEBUG_IF(VL_DBG_MSGF("+ Clock loop\n"););
        Vtop2___024root___eval(&(vlSymsp->TOP));
        if (VL_UNLIKELY(++__VclockLoop > 100)) {
            // About to fail, so enable debug to see what's not settling.
            // Note you must run make with OPT=-DVL_DEBUG for debug prints.
            int __Vsaved_debug = Verilated::debug();
            Verilated::debug(1);
            __Vchange = Vtop2___024root___change_request(&(vlSymsp->TOP));
            Verilated::debug(__Vsaved_debug);
            VL_FATAL_MT("vsrc/top2.v", 1, "",
                "Verilated model didn't converge\n"
                "- See https://verilator.org/warn/DIDNOTCONVERGE");
        } else {
            __Vchange = Vtop2___024root___change_request(&(vlSymsp->TOP));
        }
    } while (VL_UNLIKELY(__Vchange));
}

//============================================================
// Invoke final blocks

void Vtop2::final() {
    Vtop2___024root___final(&(vlSymsp->TOP));
}

//============================================================
// Utilities

VerilatedContext* Vtop2::contextp() const {
    return vlSymsp->_vm_contextp__;
}

const char* Vtop2::name() const {
    return vlSymsp->name();
}

//============================================================
// Trace configuration

void Vtop2___024root__traceInitTop(Vtop2___024root* vlSelf, VerilatedVcd* tracep);

static void traceInit(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    // Callback from tracep->open()
    Vtop2___024root* const __restrict vlSelf VL_ATTR_UNUSED = static_cast<Vtop2___024root*>(voidSelf);
    Vtop2__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    if (!vlSymsp->_vm_contextp__->calcUnusedSigs()) {
        VL_FATAL_MT(__FILE__, __LINE__, __FILE__,
            "Turning on wave traces requires Verilated::traceEverOn(true) call before time 0.");
    }
    vlSymsp->__Vm_baseCode = code;
    tracep->module(vlSymsp->name());
    tracep->scopeEscape(' ');
    Vtop2___024root__traceInitTop(vlSelf, tracep);
    tracep->scopeEscape('.');
}

void Vtop2___024root__traceRegister(Vtop2___024root* vlSelf, VerilatedVcd* tracep);

void Vtop2::trace(VerilatedVcdC* tfp, int, int) {
    tfp->spTrace()->addInitCb(&traceInit, &(vlSymsp->TOP));
    Vtop2___024root__traceRegister(&(vlSymsp->TOP), tfp->spTrace());
}
